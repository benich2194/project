...
/**
 * Class Trophy ~ represent a single Trophy, a trophy may belong to a stadium,
 * team, coach or player.
 * 
...
public class Trophy<T> implements Comparable<Trophy<T>> {
	...
	// -------------------------------More Methods------------------------------
	/**
	 * Trophy objects must be ordered by trophy's winning date
	 */
	public int compareTo(Trophy<T> o) {
		if (o.getTrophyWinningDate().before(this.trophyWinningDate)) {// compareto by winning date, if the date was
																		// before this date, return 1,if equals,return
																		// 0, else return -1
			return 1;
		}
		if (o.getTrophyWinningDate().equals(this.trophyWinningDate)) {
			return 0;
		}
		return -1;
	}
